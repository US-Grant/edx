...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        self.tiles = [ [False] * self.height for _ in range(self.width) ]
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        tmp = self.position.getNewPosition(self.direction, self.speed)
        if self.room.isPositionInRoom(tmp):
            self.position = tmp
            self.room.cleanTileAtPosition(self.position)
        else:
            self.direction = random.randint(0,359)
...
#print(runSimulation(3, 1.0, 20, 20, 1, 10, StandardRobot))
